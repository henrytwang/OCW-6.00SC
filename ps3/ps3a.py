...
def update_hand(hand, word):
    ...
    # TO DO ...
    for char in word:
        current_count = hand[char]
        hand[char] = current_count - 1
    return hand
...
def hand_exists(hand):
    exists = False
    for key in hand:
        if hand[key] is not 0:
            exists = True
    return exists
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    total_score = 0
    current_score = 0
    while hand_exists(hand):
        word = raw_input('Enter word, or a "." to indicate that you are finished: ')
        if word is ".":
            break
        if is_valid_word(word, hand, word_list):
            update_hand(hand, word)
            current_score = get_word_score(word, hand_length)
            print current_score
